...
Config parse_args(int argc, char* argv[])
{
    ...
    // set->add_option("--epp", cfg.epp, "Set Energy Performance Preference (EPP)")->check(CLI::IsMember({"performance",
    // "balance_performance", "balance_power", "power_save"}));
    ...
    monitor->add_option("-i,--interval", cfg.interval, "Set monitoring interval (seconds)")
    ...
    if (info->parsed())
        cfg.activeSubcommand = Subcommand::Info;
    else if (set->parsed())
        cfg.activeSubcommand = Subcommand::Set;
    else if (monitor->parsed())
        cfg.activeSubcommand = Subcommand::Monitor;
